...
Include "Polski";               ! The natural language definition, whose filename is taken from
...
[ NounDomain domain1 domain2 context    first_word i j k l
    ...
    for (i=1 : i<=number_of_classes : i++) {
        ...
        if (i == j-1) print (string) OR2__TX;   ! wstawione OR2 zamiast OR
    }
    L__M(##Miscellany, 57);
...
[ PrintCommand from i k spacing_flag temp_noun_form temp_second;
	...
	temp_noun_form = LanguagePreposition(i);
	...
	if (i == PATTERN_NULL) continue;
	    ...
	    if (i in compass && LanguageVerbLikesAdverb(verb_word))
                { LanguageDirection (i.door_dir); jump TokenPrinted; }; ! the direction name as adverb
            if (i == player) 
		{ print "siê"; jump TokenPrinted; };
