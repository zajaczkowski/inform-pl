...
Constant BIERNIK_BIT     256;       ! Jesli aktywny to wyswietla biernik jesli nie to mianownik (DEFART_BIT)
...
!            else {
                if (c_style & BIERNIK_BIT ~= 0) print (bie) j; else print (name) j;
!		print (bie) j;
!            }
        }
        else {
            !if (c_style & BIERNIK_BIT ~= 0) PrefaceByArticle(j, 1, sizes_p->i);
            print (number) sizes_p->i, " ";
            if (sizes_p->i > 1 && sizes_p->i < 5 ) PrintOrRun(j, plural_biernik, 1); else
            if (sizes_p->i > 20 && (sizes_p->i)%10 > 1 && (sizes_p->i)%10 < 5) PrintOrRun(j, plural_biernik, 1); else
            PrintOrRun(j, plural_dopelniacz, 1);
...
[ WriteAfterEntry o depth stack_p
    ...
    if (child_count && (c_style & RECURSE_BIT)) {
        ...
        if (o has container && (o has open || o has transparent)) {
            if (c_style & ENGLISH_BIT) {
                ...
                if (o has animate)       print (string) WHOM__TX;
                else                     print (ktorymktorejktorych) o, " ";
                }
            recurse_flag = true;
        }
    }
    ...
    if (recurse_flag && (c_style & ENGLISH_BIT))
        if (child_count > 1 || eldest_child has pluralname) 
            {
             if (child_count < 5) print (string) ARE2__TX; else print (string) IS2__TX;}
...
[ FullScoreSub i;
    ...
    new_line; L__M(##FullScore, 4); print score; L__M(##FullScore, 5);
];
...
[ InvWideSub;
    inventory_style = BIERNIK_BIT+ENGLISH_BIT+RECURSE_BIT+FULLINV_BIT;
    <Inv>;
];

[ InvTallSub;
    inventory_style = NEWLINE_BIT+RECURSE_BIT+INDENT_BIT+FULLINV_BIT+BIERNIK_BIT;
    <Inv>;
];
...
[ LockSub;
    ...
    if (noun has open)    { L__M(##Lock, 3 ,noun); give noun ~open; }
